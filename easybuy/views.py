...
def home(request):
	...
	popular_products_list = PageView.objects.filter(primary_content_type=product_type).values("primary_object_id").annotate(counter=Count("primary_object_id")).order_by("-counter")[:3]
	popular_products = []
